...
def test_learner_dashboard():
    ...
    login_url = "http://127.0.0.1:8005/auth/login"
    ...
    # Test my-courses
    my_courses_res = requests.get("http://127.0.0.1:8005/courses/my-courses", headers=headers)
    print(f"My Courses Status: {my_courses_res.status_code}")
    print(f"My Courses: {my_courses_res.json()}")
    
    # Test all courses
    all_courses_res = requests.get("http://127.0.0.1:8005/courses", headers=headers)
